...
use WpMcpUltimate\Server\Domain\Prompts\Contracts\McpPromptBuilderInterface;
use WpMcpUltimate\Server\Domain\Prompts\McpPrompt;
use WpMcpUltimate\Server\Domain\Prompts\RegisterAbilityAsMcpPrompt;
use WpMcpUltimate\Server\Domain\Resources\McpResource;
use WpMcpUltimate\Server\Domain\Resources\RegisterAbilityAsMcpResource;
use WpMcpUltimate\Server\Domain\Tools\McpTool;
use WpMcpUltimate\Server\Domain\Tools\RegisterAbilityAsMcpTool;
...
class McpComponentRegistry {
	 ...
	 * Constructor.
	 *
	 * @param \WpMcpUltimate\Server\McpServer $mcp_server MCP server instance.
	 * @param \WpMcpUltimate\Server\Infrastructure\ErrorHandling\Contracts\McpErrorHandlerInterface $error_handler Error handler instance.
	 * @param \WpMcpUltimate\Server\Infrastructure\Observability\Contracts\McpObservabilityHandlerInterface $observability_handler Observability handler instance.
	...
	public function register_tools( array $tools ): void {
		foreach ( $tools as $tool_item ) {
			...
			if ( $this->mcp_validation_enabled ) {
				...
				$validation_result = \WpMcpUltimate\Server\Domain\Tools\McpToolValidator::validate_tool_instance( $tool, $context_to_use );
	 ...
	 *
	 * @param \WpMcpUltimate\Server\Domain\Tools\McpTool $tool The tool instance to register.
	 *
	 * @return void
	 */
	...
	public function register_prompts( array $prompts ): void {
		foreach ( $prompts as $prompt_item ) {
			...
			if ( class_exists( $prompt_item ) && in_array( McpPromptBuilderInterface::class, class_implements( $prompt_item ) ?: array(), true ) ) {
				...
				try {
					/** @var \WpMcpUltimate\Server\Domain\Prompts\Contracts\McpPromptBuilderInterface $builder */
				...
				if ( $this->mcp_validation_enabled ) {
					...
					$validation_result = \WpMcpUltimate\Server\Domain\Prompts\McpPromptValidator::validate_prompt_instance( $prompt, $context_to_use );
	 ...
	 *
	 * @return \WpMcpUltimate\Server\Domain\Tools\McpTool[]
	 */
	 ...
	 *
	 * @return \WpMcpUltimate\Server\Domain\Prompts\McpPrompt[]
	 */

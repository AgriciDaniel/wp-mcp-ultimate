...
namespace WpMcpUltimate\Server\Infrastructure\ErrorHandling;
...
class McpErrorFactory {
	...
	public static function parse_error( int $id, string $details = '' ): array {
		$message = __( 'Parse error', 'wp-mcp-ultimate' );
		if ( $details ) {
			$message .= ': ' . $details;
		}
	...
	public static function invalid_request( int $id, string $details = '' ): array {
		$message = __( 'Invalid Request', 'wp-mcp-ultimate' );
		if ( $details ) {
			$message .= ': ' . $details;
		}
	...
	public static function method_not_found( int $id, string $method ): array {
		return self::create_error_response(
			...
			sprintf(
				...
				__( 'Method not found: %s', 'wp-mcp-ultimate' ),
				$method
			)
		);
	}
	...
	public static function invalid_params( int $id, string $details = '' ): array {
		$message = __( 'Invalid params', 'wp-mcp-ultimate' );
		if ( $details ) {
			$message .= ': ' . $details;
		}
	...
	public static function internal_error( int $id, string $details = '' ): array {
		$message = __( 'Internal error', 'wp-mcp-ultimate' );
		if ( $details ) {
			$message .= ': ' . $details;
		}
	...
	public static function mcp_disabled( int $id ): array {
		return self::create_error_response(
			$id,
			self::SERVER_ERROR,
			__( 'MCP functionality is currently disabled', 'wp-mcp-ultimate' )
		);
	}
	...
	public static function validation_error( int $id, string $details ): array {
		return self::create_error_response(
			...
			sprintf(
				...
				__( 'Validation error: %s', 'wp-mcp-ultimate' ),
				$details
			)
		);
	}
	...
	public static function missing_parameter( int $id, string $parameter ): array {
		return self::create_error_response(
			...
			sprintf(
				...
				__( 'Missing required parameter: %s', 'wp-mcp-ultimate' ),
				$parameter
			)
		);
	}
	...
	public static function resource_not_found( int $id, string $resource_uri ): array {
		return self::create_error_response(
			...
			sprintf(
				...
				__( 'Resource not found: %s', 'wp-mcp-ultimate' ),
				$resource_uri
			)
		);
	}
	...
	public static function tool_not_found( int $id, string $tool ): array {
		return self::create_error_response(
			...
			sprintf(
				...
				__( 'Tool not found: %s', 'wp-mcp-ultimate' ),
				$tool
			)
		);
	}
	...
	public static function ability_not_found( int $id, string $ability ): array {
		return self::create_error_response(
			...
			sprintf(
				/* translators: %s: tool name */
				__( 'Ability not found: %s', 'wp-mcp-ultimate' ),
				$ability
			)
		);
	}
	...
	public static function prompt_not_found( int $id, string $prompt ): array {
		return self::create_error_response(
			...
			sprintf(
				...
				__( 'Prompt not found: %s', 'wp-mcp-ultimate' ),
				$prompt
			)
		);
	}
	...
	public static function permission_denied( int $id, string $details = '' ): array {
		$message = __( 'Permission denied', 'wp-mcp-ultimate' );
		if ( $details ) {
			$message .= ': ' . $details;
		}
	...
	public static function unauthorized( int $id, string $details = '' ): array {
		$message = __( 'Unauthorized', 'wp-mcp-ultimate' );
		if ( $details ) {
			$message .= ': ' . $details;
		}
	...
	public static function validate_jsonrpc_message( $message ) {
		if ( ! is_array( $message ) ) {
			return self::invalid_request( 0, __( 'Message must be a JSON object', 'wp-mcp-ultimate' ) );
		}

		// Must have jsonrpc field with value "2.0".
		if ( ! isset( $message['jsonrpc'] ) || '2.0' !== $message['jsonrpc'] ) {
			return self::invalid_request( 0, __( 'jsonrpc version must be "2.0"', 'wp-mcp-ultimate' ) );
		}
		...
		if ( ! $is_request_or_notification && ! $is_response ) {
			return self::invalid_request( 0, __( 'Message must have either method or result/error field', 'wp-mcp-ultimate' ) );
		}

		// Responses must have an id field.
		if ( $is_response && ! isset( $message['id'] ) ) {
			return self::invalid_request( 0, __( 'Response messages must have an id field', 'wp-mcp-ultimate' ) );
		}

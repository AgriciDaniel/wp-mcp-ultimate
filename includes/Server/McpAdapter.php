 ...
 *
 * @package WpMcpUltimate\Server
 */

declare( strict_types=1 );

namespace WpMcpUltimate\Server;
...
use WpMcpUltimate\Server\Cli\McpCommand;
use WpMcpUltimate\Server\Infrastructure\ErrorHandling\Contracts\McpErrorHandlerInterface;
use WpMcpUltimate\Server\Infrastructure\ErrorHandling\NullMcpErrorHandler;
use WpMcpUltimate\Server\Infrastructure\Observability\Contracts\McpObservabilityHandlerInterface;
use WpMcpUltimate\Server\Infrastructure\Observability\NullMcpObservabilityHandler;
use WpMcpUltimate\Server\DefaultServerFactory;
...
final class McpAdapter {
	 ...
	 * Registry instance
	 *
	 * @var \WpMcpUltimate\Server\McpAdapter
	 */
	 ...
	 *
	 * @return \WpMcpUltimate\Server\McpAdapter
	 */
	 ...
	 * @param class-string<\WpMcpUltimate\Server\Infrastructure\ErrorHandling\Contracts\McpErrorHandlerInterface> $error_handler The error handler class name. If null, NullMcpErrorHandler will be used.
	 * @param class-string<\WpMcpUltimate\Server\Infrastructure\Observability\Contracts\McpObservabilityHandlerInterface> $observability_handler The observability handler class name. If null, NullMcpObservabilityHandler will be used.
	 ...
	 *
	 * @return \WpMcpUltimate\Server\McpAdapter|\WP_Error McpAdapter instance on success, WP_Error on failure.
	 */
	public function create_server( string $server_id, string $server_route_namespace, string $server_route, string $server_name, string $server_description, string $server_version, array $mcp_transports, ?string $error_handler, ?string $observability_handler = null, array $tools = array(), array $resources = array(), array $prompts = array(), ?callable $transport_permission_callback = null ) {
		...
		if ( ! class_exists( $error_handler ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					esc_html__( 'Error handler class "%s" does not exist.', 'wp-mcp-ultimate' ),
		...
		if ( ! in_array( McpErrorHandlerInterface::class, class_implements( $error_handler ) ?: array(), true ) ) {
			return new \WP_Error(
				...
				sprintf(
				/* translators: %s: error handler class name */
					esc_html__( 'Error handler class "%s" must implement the McpErrorHandlerInterface.', 'wp-mcp-ultimate' ),
					esc_html( $error_handler )
				)
			);
		}
		...
		if ( ! class_exists( $observability_handler ) ) {
			return new \WP_Error(
				...
				/* translators: %s: observability handler class name */
					esc_html__( 'Observability handler class "%s" does not exist.', 'wp-mcp-ultimate' ),
		...
		if ( ! in_array( McpObservabilityHandlerInterface::class, class_implements( $observability_handler ) ?: array(), true ) ) {
			return new \WP_Error(
				...
				sprintf(
				/* translators: %s: observability handler class name */
					esc_html__( 'Observability handler class "%s" must implement the McpObservabilityHandlerInterface interface.', 'wp-mcp-ultimate' ),
					esc_html( $observability_handler )
				)
			);
		}
		...
		if ( ! doing_action( 'mcp_adapter_init' ) ) {
			_doing_it_wrong(
				__FUNCTION__,
				esc_html__( 'MCP Servers must be created during the "mcp_adapter_init" action. Hook into "mcp_adapter_init" to register your server.', 'wp-mcp-ultimate' ),
				'0.1.0'
			);
			return new \WP_Error(
				'invalid_timing',
				esc_html__( 'MCP Server creation must be done during mcp_adapter_init action.', 'wp-mcp-ultimate' )
			);
		}
		...
		if ( isset( $this->servers[ $server_id ] ) ) {
			_doing_it_wrong(
				...
				// translators: %s: server ID
					esc_html__( 'Server with ID "%s" already exists. Each server must have a unique ID.', 'wp-mcp-ultimate' ),
					esc_html( $server_id )
				),
				'0.1.0'
			);
			return new \WP_Error(
				'duplicate_server_id',
				// translators: %s: server ID.
				sprintf( esc_html__( 'Server with ID "%s" already exists.', 'wp-mcp-ultimate' ), esc_html( $server_id ) )
			);
		}
	 ...
	 *
	 * @return \WpMcpUltimate\Server\McpServer|null
	 */

...
namespace WpMcpUltimate\Server\Domain\Prompts;

use WpMcpUltimate\Server\Domain\Resources\McpResourceValidator;
use WpMcpUltimate\Server\Domain\Utils\McpValidator;
...
class McpPromptValidator {
	...
	public static function validate_prompt_data( array $prompt_data, string $context = '' ) {
		...
		if ( ! empty( $validation_errors ) ) {
			...
			/* translators: %s: comma-separated list of validation errors */
				__( 'Prompt validation failed: %s', 'wp-mcp-ultimate' ),
	 ...
	 *
	 * @param \WpMcpUltimate\Server\Domain\Prompts\McpPrompt $prompt The prompt instance to validate.
	 * @param string    $context Optional context for error messages.
	 *
	 * @return bool|\WP_Error True if valid, WP_Error if validation fails.
	 */
	 ...
	 *
	 * @param \WpMcpUltimate\Server\Domain\Prompts\McpPrompt $prompt The resource instance to validate.
	 * @param string    $context Optional context for error messages.
	 *
	...
	public static function validate_prompt_uniqueness( McpPrompt $prompt, string $context = '' ) {
		...
		if ( $existing_resource ) {
			...
			/* translators: %s is the prompt name */
				__( "Prompt name '%s' is not unique. It already exists in the MCP server.", 'wp-mcp-ultimate' ),
				$this_prompt_name
			);
	...
	public static function get_validation_errors( array $prompt_data ): array {
		...
		if ( empty( $prompt_data['name'] ) || ! is_string( $prompt_data['name'] ) || ! McpValidator::validate_tool_or_prompt_name( $prompt_data['name'] ) ) {
			$errors[] = __( 'Prompt name is required and must only contain letters, numbers, hyphens (-), and underscores (_), and be 255 characters or less', 'wp-mcp-ultimate' );
		}

		// Check optional fields if present
		if ( isset( $prompt_data['title'] ) && ! is_string( $prompt_data['title'] ) ) {
			$errors[] = __( 'Prompt title must be a string if provided', 'wp-mcp-ultimate' );
		}

		if ( isset( $prompt_data['description'] ) && ! is_string( $prompt_data['description'] ) ) {
			$errors[] = __( 'Prompt description must be a string if provided', 'wp-mcp-ultimate' );
		}
		...
		if ( isset( $prompt_data['annotations'] ) ) {
			if ( ! is_array( $prompt_data['annotations'] ) ) {
				$errors[] = __( 'Prompt annotations must be an array if provided', 'wp-mcp-ultimate' );
			} else {
	...
	private static function get_arguments_validation_errors( $arguments ): array {
		...
		if ( ! is_array( $arguments ) ) {
			return array( __( 'Prompt arguments must be an array if provided', 'wp-mcp-ultimate' ) );
		}
		...
		foreach ( $arguments as $index => $argument ) {
			if ( ! is_array( $argument ) ) {
				...
				/* translators: %d: argument index */
					__( 'Prompt argument at index %d must be an object', 'wp-mcp-ultimate' ),
					$index
				);
				continue;
			}
			...
			if ( empty( $argument['name'] ) || ! is_string( $argument['name'] ) ) {
				$errors[] = sprintf(
				/* translators: %d: argument index */
					__( 'Prompt argument at index %d must have a non-empty name string', 'wp-mcp-ultimate' ),
					$index
				);
				continue;
			}
			...
			if ( ! McpValidator::validate_argument_name( $argument['name'] ) ) {
				...
				/* translators: %s: argument name */
					__( 'Prompt argument \'%s\' name must only contain letters, numbers, hyphens (-), and underscores (_), and be 64 characters or less', 'wp-mcp-ultimate' ),
					$argument['name']
				);
			}
			...
			if ( isset( $argument['description'] ) && ! is_string( $argument['description'] ) ) {
				$errors[] = sprintf(
				/* translators: %s: argument name */
					__( 'Prompt argument \'%s\' description must be a string if provided', 'wp-mcp-ultimate' ),
					$argument['name']
				);
			}
			...
			$errors[] = sprintf(
			/* translators: %s: argument name */
				__( 'Prompt argument \'%s\' required field must be a boolean if provided', 'wp-mcp-ultimate' ),
				$argument['name']
			);
		}
		...
		foreach ( $messages as $index => $message ) {
			if ( ! is_array( $message ) ) {
				...
				/* translators: %d: message index */
					__( 'Message at index %d must be an object', 'wp-mcp-ultimate' ),
					$index
				);
				continue;
			}
			...
			if ( empty( $message['role'] ) || ! is_string( $message['role'] ) ) {
				$errors[] = sprintf(
				/* translators: %d: message index */
					__( 'Message at index %d must have a role field', 'wp-mcp-ultimate' ),
					$index
				);
				continue;
			}
			...
			if ( ! in_array( $message['role'], array( 'user', 'assistant' ), true ) ) {
				$errors[] = sprintf(
				/* translators: %d: message index */
					__( 'Message at index %d role must be either \'user\' or \'assistant\'', 'wp-mcp-ultimate' ),
					$index
				);
			}
			...
			if ( empty( $message['content'] ) || ! is_array( $message['content'] ) ) {
				$errors[] = sprintf(
				/* translators: %d: message index */
					__( 'Message at index %d must have a content object', 'wp-mcp-ultimate' ),
					$index
				);
				continue;
			}
	...
	private static function get_content_validation_errors( array $content, int $message_index ): array {
		...
		if ( empty( $content['type'] ) || ! is_string( $content['type'] ) ) {
			return array(
				sprintf(
				/* translators: %d: message index */
					__( 'Message %d content must have a type field', 'wp-mcp-ultimate' ),
					$message_index
				),
			);
		}
		...
		switch ( $type ) {
			case 'text':
				if ( empty( $content['text'] ) || ! is_string( $content['text'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d text content must have a non-empty text field', 'wp-mcp-ultimate' ),
						$message_index
					);
				}
				break;
			...
			case 'image':
				if ( empty( $content['data'] ) || ! is_string( $content['data'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d image content must have a data field with base64-encoded image', 'wp-mcp-ultimate' ),
						$message_index
					);
				} elseif ( ! McpValidator::validate_base64( $content['data'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d image content data must be valid base64', 'wp-mcp-ultimate' ),
						$message_index
					);
				}
				...
				if ( empty( $content['mimeType'] ) || ! is_string( $content['mimeType'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d image content must have a mimeType field', 'wp-mcp-ultimate' ),
						$message_index
					);
				} elseif ( ! McpValidator::validate_image_mime_type( $content['mimeType'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d image content must have a valid image MIME type', 'wp-mcp-ultimate' ),
						$message_index
					);
				}
				break;
			...
			case 'audio':
				if ( empty( $content['data'] ) || ! is_string( $content['data'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d audio content must have a data field with base64-encoded audio', 'wp-mcp-ultimate' ),
						$message_index
					);
				} elseif ( ! McpValidator::validate_base64( $content['data'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d audio content data must be valid base64', 'wp-mcp-ultimate' ),
						$message_index
					);
				}
				...
				if ( empty( $content['mimeType'] ) || ! is_string( $content['mimeType'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d audio content must have a mimeType field', 'wp-mcp-ultimate' ),
						$message_index
					);
				} elseif ( ! McpValidator::validate_audio_mime_type( $content['mimeType'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d audio content must have a valid audio MIME type', 'wp-mcp-ultimate' ),
						$message_index
					);
				}
				break;
			...
			case 'resource':
				if ( empty( $content['resource'] ) || ! is_array( $content['resource'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d resource content must have a resource object', 'wp-mcp-ultimate' ),
						$message_index
					);
				} else {
					...
					foreach ( $resource_errors as $resource_error ) {
						...
						/* translators: %1$d: message index, %2$s: resource error */
							__( 'Message %1$d embedded resource: %2$s', 'wp-mcp-ultimate' ),
							$message_index,
							$resource_error
						);
					}
			...
			default:
				...
				/* translators: %1$d: message index, %2$s: content type */
					__( 'Message %1$d content type \'%2$s\' is not supported. Must be \'text\', \'image\', \'audio\', or \'resource\'', 'wp-mcp-ultimate' ),
					$message_index,
					$type
				);
				break;
		...
		if ( isset( $content['annotations'] ) ) {
			if ( ! is_array( $content['annotations'] ) ) {
				$errors[] = sprintf(
				/* translators: %d: message index */
					__( 'Message %d content annotations must be an array if provided', 'wp-mcp-ultimate' ),
					$message_index
				);
			} else {

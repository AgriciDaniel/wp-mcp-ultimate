...
namespace WpMcpUltimate\Server\Domain\Resources;
...
class McpResourceValidator {
	...
	public static function validate_resource_data( array $resource_data, string $context = '' ) {
		...
		if ( ! empty( $validation_errors ) ) {
			...
			/* translators: %s: comma-separated list of validation errors */
				__( 'Resource validation failed: %s', 'wp-mcp-ultimate' ),
	 ...
	 *
	 * @param \WpMcpUltimate\Server\Domain\Resources\McpResource $the_resource The resource instance to validate.
	 * @param string      $context Optional context for error messages.
	 *
	 * @return bool|\WP_Error True if valid, WP_Error if validation fails.
	 */
	...
	public static function get_validation_errors( array $resource_data ): array {
		...
		if ( empty( $resource_data['uri'] ) || ! is_string( $resource_data['uri'] ) ) {
			$errors[] = __( 'Resource URI is required and must be a non-empty string', 'wp-mcp-ultimate' );
		} elseif ( ! McpValidator::validate_resource_uri( $resource_data['uri'] ) ) {
			$errors[] = __( 'Resource URI must be a valid URI format', 'wp-mcp-ultimate' );
		}
		...
		if ( ! $has_text && ! $has_blob ) {
			$errors[] = __( 'Resource must have either text or blob content', 'wp-mcp-ultimate' );
		} elseif ( $has_text && $has_blob ) {
			$errors[] = __( 'Resource cannot have both text and blob content - only one is allowed', 'wp-mcp-ultimate' );
		}

		// Validate text content if present.
		if ( $has_text && ! is_string( $resource_data['text'] ) ) {
			$errors[] = __( 'Resource text content must be a string', 'wp-mcp-ultimate' );
		}

		// Validate blob content if present.
		if ( $has_blob && ! is_string( $resource_data['blob'] ) ) {
			$errors[] = __( 'Resource blob content must be a string (base64-encoded)', 'wp-mcp-ultimate' );
		}

		// Validate optional fields if present.
		if ( isset( $resource_data['name'] ) && ! is_string( $resource_data['name'] ) ) {
			$errors[] = __( 'Resource name must be a string if provided', 'wp-mcp-ultimate' );
		}

		if ( isset( $resource_data['description'] ) && ! is_string( $resource_data['description'] ) ) {
			$errors[] = __( 'Resource description must be a string if provided', 'wp-mcp-ultimate' );
		}

		if ( isset( $resource_data['mimeType'] ) ) {
			if ( ! is_string( $resource_data['mimeType'] ) ) {
				$errors[] = __( 'Resource mimeType must be a string if provided', 'wp-mcp-ultimate' );
			} elseif ( ! McpValidator::validate_mime_type( $resource_data['mimeType'] ) ) {
				$errors[] = __( 'Resource mimeType must be a valid MIME type format', 'wp-mcp-ultimate' );
			}
		}
		...
		if ( isset( $resource_data['annotations'] ) ) {
			if ( ! is_array( $resource_data['annotations'] ) ) {
				$errors[] = __( 'Resource annotations must be an array if provided', 'wp-mcp-ultimate' );
			} else {
	...
	public static function validate_resource_uniqueness( McpResource $the_resource, string $context = '' ) {
		...
		if ( $existing_resource ) {
			...
			/* translators: %s: resource URI */
				__( 'Resource URI \'%s\' is not unique. It already exists in the MCP server.', 'wp-mcp-ultimate' ),
				$this_resource_uri
			);

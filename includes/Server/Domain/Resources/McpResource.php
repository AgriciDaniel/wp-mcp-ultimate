...
class McpResource {
	 ...
	 *
	 * @var \WpMcpUltimate\Server\Core\McpServer|null
	 */
		...
		if ( ! $ability ) {
			return new \WP_Error(
				...
				sprintf(
					...
					esc_html__( "WordPress ability '%s' does not exist.", 'wp-mcp-ultimate' ),
	 ...
	 *
	 * @return \WpMcpUltimate\Server\Core\McpServer
	 */
	 ...
	 *
	 * @return \WpMcpUltimate\Server\Domain\Resources\McpResource|\WP_Error Resource instance on success, WP_Error on failure.
	 */
	public function validate( string $context = '' ) {
		if ( null === $this->mcp_server ) {
			return new \WP_Error(
				'resource_missing_mcp_server',
				esc_html__( 'MCP server must be set before validating a resource.', 'wp-mcp-ultimate' )
			);
		}

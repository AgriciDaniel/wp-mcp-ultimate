...
use WpMcpUltimate\Server\Transport\Contracts\McpTransportInterface;
		...
		foreach ( $mcp_transports as $mcp_transport ) {
			...
			if ( ! class_exists( $mcp_transport ) ) {
				_doing_it_wrong(
					...
					sprintf(
						...
						esc_html__( 'Transport class "%s" does not exist. Make sure the class is properly autoloaded or included.', 'wp-mcp-ultimate' ),
			...
			if ( ! in_array( McpTransportInterface::class, class_implements( $mcp_transport ) ?: array(), true ) ) {
				_doing_it_wrong(
					...
					sprintf(
						/* translators: %s: Transport class name */
						esc_html__( 'Transport class "%s" must implement McpTransportInterface. Check your transport implementation.', 'wp-mcp-ultimate' ),
						esc_html( $mcp_transport )
					),
					'0.1.0'
				);
